...
export default function ChatPage() {
  ...
  const fetchMessages = useCallback(async (pageNumber = 1) => {
    ...
    try {
      ...
      const res = await fetch(`https://rendering-dual-pharmacology-obligations.trycloudflare.com/api/conversations?user_id=${userId}&page=${pageNumber}`);
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      setBotTyping(true);
      const userId = await AsyncStorage.getItem("userId");
      const res = await fetch("https://rendering-dual-pharmacology-obligations.trycloudflare.com/api/generateText", {
        method: "POST",
